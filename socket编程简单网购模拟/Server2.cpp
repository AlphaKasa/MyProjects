...
public:
    ...
    void ServerFunction(int nConnectedSocket, int nListenSocket)
    {
        ...
        s2.CetPluginNames();
        s2.InitializeCommodity();

        Serializer ser;
		ser.Initialization();  
		ser.Trans(2,s2.AllCommodity);
        ...
        while(id!=0){
            ...
            float money=s2.Sell(id,num);
            memcpy(send_buffer ,&money,sizeof(float));
            send(nConnectedSocket,send_buffer,sizeof(float),0);
            delete(send_buffer);
        }
		cout<<"输入1将商品全部转移至服务器1，输入2将转移的商品撤销，输入其他退出服务器:"<<endl;
		int ins=0;
		cin>>ins;
		if(ins==1){
            FILE *fp=fopen("data2","w+");
			int num=s2.AllCommodity.size();
			fwrite(&num,sizeof(int),1,fp);
			for(int i=0;i<num;i++){
			     fwrite(&s2.AllCommodity[i]->id,sizeof(int),1,fp);
			}
			fclose(fp);
		}
        else if(ins==2){
...
int main()
{
    CTCPServer<CMyTCPServer2> server2(4000);
    server2.RunFuction();
    return 0;
}
